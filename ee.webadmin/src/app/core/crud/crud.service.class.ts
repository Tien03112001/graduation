...
import {getBaseHref, StorageUtil} from '../utils';
...
export class AbstractCRUDService<T> {
  ...
  setApiUrl(newPrefixUrl: string) {
    if (newPrefixUrl) {
      this.urlRestAPI = AbstractCRUDService.pathJoin([newPrefixUrl, this.urlRestAPI]);
    }
  }
    ...
    if (err.error instanceof Error) {
      ...
      if (err.status == 401) {
        ...
        if (err.status >= 400 && err.status < 500) {
          ...
          if (err.status >= 500) {
            this.toastError('Hệ thống phản hồi', `Lỗi xảy ra trên hệ thống`);
          } else {
            this.toastError('Hệ thống phản hồi', `Không xác định lỗi`);
          }
        }
      }
    }
  ...
  private static pathJoin(parts: string[], sep: string = '/') {
    const separator = sep || '/';
