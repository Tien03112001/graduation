...
@section('title',isset($category->meta)?$category->meta->title:'')
@if(isset($category->meta))
    @push('meta_data')
        @isset ($category->meta->description)
            <meta name="description" content="{{$category->meta->description}}">
        @endisset
        @isset ($category->meta->keywords)
            <meta name="keywords" content="{{$category->meta->keywords}}">
        @endisset
        @isset ($category->meta->robots)
            <meta name="robots" content="{{$category->meta->robots}}">
        @endisset
        @isset ($category->meta->canonical)
            <link rel="canonical" href="{{$category->meta->canonical}}"/>
        @endisset
    @endpush
@endif
...
@section('structure_data')
    @forelse($category->structured_datas as $s)
