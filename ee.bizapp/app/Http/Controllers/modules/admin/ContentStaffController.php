...
class ContentStaffController extends Controller implements ApiController
{
    public function index(Request $request)
    {
        $search = $request->input('search', null);
        $limit = $request->input('limit', null);
        $query = (new ContentStaff())
    ...
    public function update(Request $request, $id)
    {
        try {
            DB::beginTransaction();
            $model = (new ContentStaff())->find($id);
            $model->name = $request->input('name');
            $model->message = $request->input('message', '');
            if ($request->hasFile('image')) {
                $url = Storage::disk('public')->put('images', $request->file('image'));
