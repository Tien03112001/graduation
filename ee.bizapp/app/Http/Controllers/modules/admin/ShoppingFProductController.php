...
use App\Models\GProductShopping;
use App\Models\ShoppingFProduct;
...
class ShoppingFProductController extends Controller implements ApiController
{
    public function index(Request $request)
    {
        $search = $request->input('search', null);
        $limit = $request->input('limit', null);
        $query = (new ShoppingFProduct())
    ...
    {
        $products = ContentProduct::with('f_shopping', 'inventories', 'category', 'tags')->get();
        try {
            DB::beginTransaction();
            foreach ($products as $product) {
                if ($product->f_shopping) {
                    $model = $product->f_shopping;
                } else {
                    $model = new ShoppingFProduct();
                }
                $model = ShoppingUtil::fill_f_product($product, $model);
                $model->save();
            }
            DB::commit();
            return $this->success([]);
    ...
    {
        $model = (new ShoppingFProduct())->find($id);
        if (empty($model)) {
            return $this->error('Not Found');
        }
        $model->fid = $request->fid;
        ...
        $model->fb_product_category = $request->fb_product_category;
        $model->sale_price_effective_date = $request->sale_price_effective_date;
    ...
    {
        $model = ShoppingFProduct::find($id);
        if (empty($model)) {
            return $this->error('Not found');
        }
        $model->delete();
    ...
    public function import(Request $request)
    {
        ...
        $data = OfficeUtil::readXLSX($file->getRealPath(), 0, 1, 'A', -1, '');
        if (empty($data)) {
            return $this->error('Không đúng định dạng file');
        }
        $columns = $data[0];
        ...
        foreach ($gProducts as $g) {
            try {
                ...
                $model = ShoppingFProduct::find($g['id']);
                if (empty($model)) {
                ...
                $model->fb_product_category = $g['fb_product_category'];
                $model->sale_price_effective_date = $g['sale_price_effective_date'];
    ...
    {
        if (ShoppingUtil::generate_facebook_xml()) {
            return $this->success([]);
        } else {
