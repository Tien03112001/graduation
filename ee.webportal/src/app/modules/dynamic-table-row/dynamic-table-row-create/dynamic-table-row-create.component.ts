...
export class DynamicTableRowCreateComponent extends AbstractModalComponent<any> {
  ...
  loaded(): void {
    for (let i = 0; i < this.model.columns.length; i++) {
      let formControlName = `column${this.model.columns[i].id}`;
      this.formGroup.addControl(formControlName, new FormControl(null, [Validators.required, , Validators.min(0)]));
      ...
      if (this.model.columns[i].type === 'image') {
        ...
        fieldForm = FieldForm.createTextInput(this.model.columns[i].name, formControlName, 'Nhập thông dữ liệu');
  ...
  createWithImage() {
    let item: any = {
      table_id: +this.formGroup.controls['id'].value,
    };
    for (let i = 0; i < this.model.columns.length; i++) {
      let formControlName = `column${this.model.columns[i].id}`;
      item[formControlName] = this.formGroup.controls[formControlName].value;
    }
