...
use App\Common\Core\KafkaTopicConfigCollection;
...
class KafkaUtil extends SingletonPattern
{
     ...
     * @param KafkaTopicConfig[] $topicConfig
     * @param KafkaClosure $closure
     * @return void
     * @throws \Exception
     */
    public function consume(string $groupId, $topicConfig, KafkaClosure $closure)
    {
        ...
        foreach ($topicConfig as $t) {
            $topicConf = new RdKafkaTopicConf();
            foreach ($t->getConfigs() as $key => $value) {
                if (isset($value)) {
                    $topicConf->set($key, $value);
                }
            }
            $topic = $consumer->newTopic($t->getName(), $topicConf);
            foreach ($t->getPartitions() as $p) {
                $topic->consumeQueueStart($p->getPartition(), $p->getOffset(), $queue);
            }
        }
            ...
            if (isset($message)) {
                switch ($message->err) {
                    case RD_KAFKA_RESP_ERR_NO_ERROR:
                        $closure->getSuccess()($message);
                        $consumer->poll(100);
                        break;
                    case RD_KAFKA_RESP_ERR__PARTITION_EOF:
                        $closure->getPartitionOffError()($message);
                        break;
                    case RD_KAFKA_RESP_ERR__TIMED_OUT:
                        $closure->getTimeOutError()($message);
                        break;
                    default:
                        $closure->getException()($message);
                        break;
                }
            }
        }
