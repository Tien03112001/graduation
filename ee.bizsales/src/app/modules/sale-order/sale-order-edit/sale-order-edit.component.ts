...
@Component({
  ...
  providers: [PromotionService, SaleOrderService, SaleCustomerService, ChannelService, ShippingProvinceService, PaymentTypeService, VoucherService, ProductService, DefinitionService]
})
export class SaleOrderEditComponent extends AbstractModalComponent<SaleOrderMeta> {
  ...
  loadAllPromotion() {
    return this.promotionService.loadAll();
  }
  ...
  initFieldForm(): FieldForm[] {
    return [
      ...
      FieldForm.createNumberInput('Mã giảm giá', 'promotion', '0'),
    ];
  }
  ...
  addProduct(productList: ProductMeta[]) {
    if (productList && productList.length > 0) {
      this.arrProduct.push({
        ...
        variant_id: 0,
        unit_price: productList[0].price,
        quantity: 1,
      });
      this.updateCart();
    }
  ...
  updateCart() {
    ...
    this.promotionService.loadAll().subscribe((res: any) => {
      for (let i of res) {
        if (i.type == 4) {
          if (amount > i.min_order_value) {
            amount += +this.formGroup.get('ship_fee').value -i.discount_value;
          }
        }
      }
      this.formGroup.controls['amount'].setValue(amount);
